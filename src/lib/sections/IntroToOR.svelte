...
<BodyText>
    If you're like me, one of the first things you'll do when learning a new subject is look it up on Wikipedia. As of this writing, <a href='https://en.wikipedia.org/wiki/Operations_research'>their page on Operations Research</a> first defines OR as:
</BodyText>
...
<BodyText>
    Right. So the practice of OR involves using mathematical techniques to find the best decision possible in a given situation ("optimal" is just fancy way of saying "best"<Footnote>The pedant in me wants to point out that this word "optimal" is horribly misused in conversation fairly regularly, in the form of the misguided phrase "more optimal". As I mentioned, "optimal" really just means "best", and "best" is not on a sliding scale; either you have the best answer or you don't. One choice can't be "more best" than another, just as one solution can't be "more optimal" than another. I think I'm fighting a losing battle on this, but maybe for the sake of this course we can all agree to never put the words "more" and "optimal" next to each other? Thanks.</Footnote>).
</BodyText>

<Heading level=2 refId=exampleORProbs>Example OR problems</Heading>
...
<Heading level=3 refId=tsp>Traveling Salesman Problem</Heading>

<BodyText>
    There are many well-known problems in the world of OR, but I reckon the <a href='https://en.wikipedia.org/wiki/Travelling_salesman_problem'>Traveling Salesman Problem</a> (TSP) is the best-known and most-loved among them. The setup is simple: Some old-timey door-to-door salesman is heading out on the road to sell his product to the masses. He plans to visit a certain group of cities, and thanks to his trusty atlas he knows the way between any pair of cities. Less clear, however, is the shortest path that will lead you through _all_ of the cities, and this is the aim of the TSP: In which order should you visit the cities such that your total distance traveled is minimized?
</BodyText>

<BodyText>
    As I said, this a famous problem in the OR space<Footnote>There's a neat book all about the TSP, <CitationRef refId=tspPursuit/>, written by a great professor that I took a course from while getting my PhD. It's more popularly focused than technical, so it's a surprisingly smooth read. Highly recommended!</Footnote>. I think it's due to the simple, relatable exposition, paired with the fact that it is actually quite computationally challenging. And yet despite the challenges, modern methods are able to solve problem instances where the number of cities is in the 10,000s! The image below shows the optimal tour through selected cities in the continental US<Footnote>You can tell from the image that this map is pretty old. <a href='https://www.math.uwaterloo.ca/tsp/usa50/'>The site from which it came</a> tells a neat story about how this instance was solved, by hand, way back in 1954! There are some other interesting bits there too, well worth a read in my opinion.</Footnote>.
</BodyText>
...
<Heading level=3 refId=jobShop>Job-shop scheduling</Heading>
...
<Heading level=3 refId=portfolioOpt>Portfolio optimization</Heading>
...
<Heading level=2 refId=orInPractice>OR in practice</Heading>
...
<Heading level=3 refId=caseStudies>Case Studies</Heading>
...
<Figure refId="caseStudies">
    <img src={caseStudies} alt="Case studies" />
    <span slot=caption>Selected OR case studies, from <CitationRef refId=classText/></span>
</Figure>
...
<Heading level=3 refId=edelmanPrize>Edelman Prize</Heading>

<BodyText>
    Now, admittedly, some of those case studies are a little stale. But don't fret, OR is still relevant in industry today. A great showcase for the most recent impactful OR work is the annual Edelman Prize, awarded by the <a href='https://www.informs.org/'>Institute for Operations Research and Management Science (INFORMS)</a>. The winners of this award were judged to have demonstrated the best application of OR methodologies in industry. You can take a look at the <a href='https://3449182.fs1.hubspotusercontent-na1.net/hubfs/3449182/2023_Edelman_Gala_Book.pdf'>program for the 2023 edition of the award</a> and find cases submitted by names like DHL, Huawei, Lyft, and the winner Walmart.
</BodyText>

<Heading level=2 refId=classTopics>Topics we'll cover</Heading>

<BodyText>
    OR is a wide-ranging topic, and as such we can't cover everything. Since the class is meant to be an overview, we won't get overly deep into any one topic either<Footnote>If you _are_ looking for a deep dive, I'd suggest checking out the other classes in our Masters of Operations Research program.</Footnote>. But we should be able to cover the basics in a handful of important topics, as well as getting you some hand-on experience using these methods to solve problems. I've outlined the planned programming below, but note that we are still early in the semester, so some of this may be subject to change.
</BodyText>
