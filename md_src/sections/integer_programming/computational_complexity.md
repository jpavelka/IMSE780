...
While the main question in the field, $\P\stackrel{?}{=}\NP$, remains unsolved, there are still interesting things to be said about problems inside $\NP$. One of the most important notions is that of $\NP$-completeness. Central to this theory is the notion of a __problem reduction__, which is a way of taking an instance of one problem, applying some "small" (polynomially many) number of tweaks to it, then recovering an instance of a new problem. A problem is said to be __$\NP$-complete__ if it is in $\NP$ and additionally there exists a way to reduce any other $\NP$ problem to it[^npCompleteAnyOtherProblem].
